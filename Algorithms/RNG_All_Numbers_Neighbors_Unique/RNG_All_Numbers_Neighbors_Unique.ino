...
byte resLower;          // Lower bound for resevoir sampling
byte resUpper;          // Upper bound for resevoir sampling
...
void shiftOutLeft(byte pos){
  for(int i=pos; i<resUpper; i++){ // Starting at index=pos, for each index of the array to the upper bound...
    resArr[i] = resArr[i+1];      // Shift index on the right to the current index.
  }
  resUpper--;                      // Decrement upper bound by 1 to ignore useless data
}
...
void shiftOutRight(byte pos){
  for(int i=pos-1; i>=resLower; i--){ // Starting at index=pos-1, for each index of the array to resLower...
    resArr[i+1] = resArr[i];         // Shift index on the current index to the right.
  }
  resLower++;                         // Increment lower bound by 1 to ignore useless data
}
 ...
 * upper bound. Generates a random number between the lower and upper bound and performs a single resevoir sample. Returns a post processed
 * resevoir sampling array and new lower and upper bounds.
 * 
 ...
 */
byte resSort(){
  byte pos = random(resLower, resUpper+1);              // Randomly select an index of a number to pull for resevoir sampling
  byte num = resArr[pos];                                // Store that number
  if(resArr[pos - resLower] < resArr[resUpper - pos]) // If that number is closer to the left side of the resevoir sampling array...
    shiftOutRight(pos);                                   // Shift resevoir sampling array right
  else 
    shiftOutLeft(pos);                                    // Otherwise, shift resevoir sampling array left
...
void generatePattern(){
  resUpper = uniqueNum-1;          // Initialize upper bound of resevoir sampling
  resLower = 0;                    // Initialize lower bound of resevoir sampling
  ...
  for(byte i=0; i<uniqueNum; i++){ // For each unique number...
    solution[i] = resSort();       // Perform resevoir sampling and store the selected number
